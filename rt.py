...
class CrackDatasetRaw(Dataset):
    def __init__(self, image_dir, mask_dir, image_size=(50, 50), raw_size=(100, 460), transform=None):
        """
        ...
        - image_size (tuple): 画像のリサイズ先のサイズ (高さ, 幅)
        - raw_size (tuple): RAW画像の元サイズ (高さ, 幅)
    ...
    def __getitem__(self, idx):
        ...
        raw_image = load_raw_image(img_path, *self.raw_size)

        # マスク画像を読み込み
        mask_path = os.path.join(self.mask_dir, self.mask_files[idx])
        mask = Image.open(mask_path).convert("L")  # マスクはグレースケール
        
        # RAW画像をリサイズして正規化
        raw_image = cv2.resize(raw_image, self.image_size, interpolation=cv2.INTER_NEAREST)
        ...
        # Tensorに変換
        raw_image = torch.from_numpy(raw_image).unsqueeze(0)  # [C, H, W]の形式にする
        mask = torch.from_numpy(np.array(mask)).unsqueeze(0)  # [C, H, W]形式に変換
...
train_dataset = CrackDatasetRaw(
    ...
    image_size=(50, 50),               # モデルの入力サイズ
    raw_size=(100, 460)                # RAW画像の元サイズ
)
...
def split_and_merge_with_inference(image, model, device, grid_size=50):
    """
    ...
    - grid_size (int): 分割するグリッドのサイズ (デフォルト: 50)
...
# 推論と再結合
predicted_image = split_and_merge_with_inference(test_image, model, device, grid_size=50)
