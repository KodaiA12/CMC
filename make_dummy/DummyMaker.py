...
#         # 菱形のような引張変形パターンを作成
#         material_mask, image = create_diamond_shape_background(
#             width, height, 
...
#             if np.any(material_column):
#                 # 素材がある場所を特定
#                 material_indices = np.where(material_column > 0)[0]
...
#                     if max_possible_length > 1:  # 少なくとも2以上の長さがある場合
#                         low = min(crack_length_range[0], max_possible_length - 1)
#                         high = min(crack_length_range[1], max_possible_length)
#                         if low < high:  # low が high より小さいことを確認
#                             crack_length = np.random.randint(low, high)
#                         else:
#                             crack_length = low  # low と high が同じ場合は単に low を使用
#                     else:
#                         # 素材が非常に小さい場合はクラックを描画しない
#                         continue  # 次のクラックに進む
...
# def create_diamond_shape_background(width, height, material_intensity_range, void_intensity_range):
#     """
#     引張変形パターン（菱形型）の背景を作成する関数
#     """
#     # まず素材がない部分（黒）の画像を作成
...
#         [center_x, top_y],      # 上端
#         [right_x, center_y],    # 右端
#         [center_x, bottom_y]    # 下端
#     ], dtype=np.int32)
    
#     # 菱形の領域を塗りつぶす
#     cv2.fillPoly(material_mask, [points], 1.0)
...
#         num_images=5, 
#         save_path="./synthetic_crack_dataset",
#         material_intensity_range=(250, 320),
...
if __name__ == "__main__":
    generate_synthetic_crack_image(
        num_images=5, 
        save_path="./synthetic_crack_dataset2",
        material_intensity_range=(250, 320),
        void_intensity_range=(115, 170),
        crack_intensity_range=(350, 600),
        crack_thickness_range=(1, 5)
